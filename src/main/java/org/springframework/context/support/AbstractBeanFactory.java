...
public abstract class AbstractBeanFactory implements ConfigurableBeanFactory {
    ...
    public Object getBean(String name) throws BeansException{
        ...
        if (sharedInstance != null) {
            log.debug("캐싱된 싱글톤 빈 {} 을 리턴합니다.", beanName);
            ...
            try{
                ...
                log.debug("부모를 뒤져봐! get Parent? {}", parentBeanFactory);
                // 없을때는 부모를 뒤져본다.
            ...
            if (mergedBeanDefinition.isSingleton()) {
                log.debug("겟빈과정에서 싱글톤을 싱글톤 캐시에 등록합니다. 빈 : {} ", mergedBeanDefinition);
    ...
    protected Object getObjectForSharedInstance(String name, Object beanInstance) {
        log.debug("name : {} 과 빈인스턴스 : {} 으로 공유인스턴스를 위한 겟오브젝트 실시 : ", name, beanInstance);
        String beanName = transformedBeanName(name);
